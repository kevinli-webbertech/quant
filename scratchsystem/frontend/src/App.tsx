...
import {
  ...
  Paper, Divider, InputLabel, IconButton, Collapse
} from "@mui/material";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import ExpandLessIcon from "@mui/icons-material/ExpandLess";
import { Link } from "react-router-dom";
import { SymbolEntry, SearchResult } from "./types";
import { createSymbol, searchSymbols, deleteSymbol } from "./api";
...
function App() {
  ...
  const handleSearch = async () => {
    ...
    if (!Array.isArray(data)) {
      console.error("Search failed:", (data as any)?.error || "Unexpected error");
      return;
    }
  ...
  return (
    <Box sx={{ bgcolor: "#f5f5f5", minHeight: "100vh", py: 4 }}>
      <Paper elevation={3} sx={{ maxWidth: 700, mx: "auto", p: 4 }}>
        <Typography variant="h4" align="center" gutterBottom>Scratch System</Typography>
        <form onSubmit={handleSubmit}>
          <Box display="flex" flexDirection="column" gap={2}>
            <TextField label="Title" name="title" value={form.title} onChange={handleInputChange} error={errors.title} helperText={errors.title ? "Title is required." : ""} fullWidth required />
            <TextField label="Body" name="body" value={form.body} onChange={handleInputChange} fullWidth multiline rows={4} sx={{ overflowY: "auto" }} />
            <TextField label="Comment" name="comment" value={form.comment} onChange={handleInputChange} fullWidth multiline rows={4} sx={{ overflowY: "auto" }} />
            <TextField type="datetime-local" name="due_date" value={form.due_date} onChange={handleInputChange} fullWidth label="Event Time" InputLabelProps={{ shrink: true }} />
            <Box>
              <InputLabel id="category-label">Category</InputLabel>
              <Select labelId="category-label" name="category" value={form.category} onChange={handleSelectChange} fullWidth>
            ...
            <Box>
              ...
              <Select labelId="priority-label" value={form.priority} name="priority" onChange={handleSelectChange} fullWidth>
            ...
            <TextField label="Tags (comma-separated)" name="tags" value={form.tags.join(", ")} onChange={handleInputChange} error={errors.tags} helperText={errors.tags ? "Please enter at least one tag." : ""} fullWidth />
